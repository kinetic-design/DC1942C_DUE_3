...
// Linduino QUIKEVAL pins now mapped to GEVCU
#define QUIKEVAL_GPIO 9//!< Linduino QuikEval GPIO pin (QuikEval connector pin 14) connects to Arduino pin 9
#define SPI_CS A9 //A9-DUE 9-GEVCU, 10 CS (SPI chip select QuikEval connector pin 6) connects to Arduino SS pin.
...
Additionally, I2C port's SDA and the SPI port's MOSI signals share same pin on Linduino's QuikEval connector.
...
//! @{
//! Arduino/Linduino is Little Endian, least significant byte is stored in first byte of larger data types.
#ifdef BIG_ENDIAN
